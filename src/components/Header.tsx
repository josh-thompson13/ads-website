...
export function Header() {
  ...
  const linkClass = transparent
    ? "text-white/90 hover:text-white relative after:absolute after:left-0 after:-bottom-1 after:h-0.5 after:w-0 hover:after:w-full after:bg-white after:transition-[width]"
    : "text-ink/80 hover:text-ink relative after:absolute after:left-0 after:-bottom-1 after:h-0.5 after:w-0 hover:after:w-full after:bg-ink after:transition-[width]";
    ...
    <header className={`sticky top-0 z-50 transition-colors duration-300 ${headerClass}`}>
      <div className="w-full px-4 md:px-6 py-3 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2" aria-label="Australian Drone Solutions home">
          ...
          <span className={`font-display font-bold tracking-tight ${transparent ? "text-white" : ""}`}>Australian Drone Solutions</span>
        </Link>
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          className={`lg:hidden inline-flex h-10 w-10 items-center justify-center rounded-lg border ${transparent ? "border-white/40 text-white" : "border-black/10"}`}
        >
