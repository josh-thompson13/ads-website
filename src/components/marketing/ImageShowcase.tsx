    ...
    <div className="w-full">
      <div className="relative h-[380px] md:h-[460px] w-full">
        ...
        <div className="absolute left-6 bottom-6 text-white max-w-xl">
          ...
          <h3 className="mt-3 text-3xl font-display font-bold">Drone Technology</h3>
          <p className="text-white/90">Real work in real conditions — consistent coverage and precise application across a variety of terrain.</p>
          <Link href="/services" className="mt-3 inline-flex items-center gap-2 rounded-xl border border-white/40 bg-white/10 px-4 py-2 text-white backdrop-blur">Learn More</Link>
        </div>
      </div>
      <div className="bg-white p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {thumbs.slice(0, 2).map((src) => (
          <div key={src} className="relative h-44 w-full overflow-hidden rounded-xl border">
